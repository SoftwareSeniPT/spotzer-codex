# Spotzer Rate Card
<?php
echo "<p>Copyright &copy; 2016-" . date("Y") . " W3Schools.com</p>";
?>
## Spotzer AU 
<img src="/images/spotzerAU_200px.png" align"left"/>
### Basic Package
|  **Item / Description** | **Software Seni AUD ** |
|  ------ | :------ |
|   |  |
|  Custom/Normal Wordpress 10 Pages (4 layouts) | $420 |
|  Custom/Normal Wordpress 20 Pages (4 layouts) | $750 |
|  Backend in a non English language  | Increases (above) 10% |
|   |  |
|  Additional Pages | $7 |
|  Additional 10 pages | $50 |
|  Additional 20 pages | $100 |
|   |  |
|  Additional Layout (assumes design is provided) | $264 |
|  Note: Excludes any plugin adjustments/creation |  |
|   |  |
### eCommerce
|  **Item / Description** | **Software Seni AUD ** |
|  ------ | :------ |
|  Woo Commerce extension with 10 products configured | $650 |
|  Per eCom product:  | $5 |
|  Per 10 eCom products:  | $30 |
|  CSV bulk import 0 - 500 products | $308 |
|  CSV bulk import 501 - 4000 products | $352 |
|  CSV bulk import 4001+ | $528 |
|   |  |
|  Shopify Account/plugin provided | $ |
|  Shopify Account/plugin not provided | Client should apply |
|  Shopify styling | Hourly |
|   |  |
### Other Services / Plugins
|  **Item / Description** | **Software Seni AUD ** |
|  ------ | :------ |
|  Additional 3 plugins (from list)  | Hourly |
|   |  |
|   |  |
|  Blog - Per page migration:  | $ |
|  Blog - Per 5 - 15 pages migration: Per page | $ |
|  Blog - Per 16 - 40 pages migration: Per page | $ |
|  Any paid version of Woo Themes/ Woo Commerce plugin | Provided by Spotzer |
|  Production Resources /Hour | $22 |
|  Pretty permalinks with product names |  |
|   |  |
|  Ongoing site maintenaince edits/updates |  |
|  By the hour:(minimum 1 hours work) | just hourly rate above |
|  4 hours per month:  | just hourly rate above |
|  8 hours per month:  | just hourly rate above |
|   |  |
### Hosting
|  **Item / Description** | **Software Seni AUD ** |
|  ------ | :------ |
|  Hosting | $8 per month |
|  Migration | $11 per site migrated (subject to changed, based on process duration) |



## Spotzer EU 
<img src="/images/spotzerEU_200px.png" align"left"/>
### Basic Package
|  **Item / Description** | **Software Seni USD ** |
|  ------ | :------ |
|   |  |
|  1 page landing page (1 Layout) | $105 |
|  3-5 page website (3 Layouts) | $210 |
|  6-10 page website (4 Layouts) | $315 |
|  11-20 page website (4 Layouts) | $563 |
|  20-50 page website (5 Layouts) | $925 |
|  Backend in a non English language  | Increases (above) 10% |
|   |  |
|  Additional Pages |  |
|  Additional 10 pages | $38 |
|  Additional 20 pages | $70 |
|   |  |
|  Additional Layout (assumes design is provided) | $204 |
|  Note: Excludes any plugin adjustments/creation |  |
|   |  |
### eCommerce
|  **Item / Description** | **Software Seni USD ** |
|  ------ | :------ |
|  Woo Commerce extension with 10 products configured | $488 |
|  Per eCom product:  | $3.75 |
|  Per 10 eCom products:  | $22.5 |
|  Bulk product ingestion | ?? |
|   |  |
|  Shopify Account/plugin provided | $136 |
|  Shopify Account/plugin not provided | Client should apply |
|  Shopify styling | Hourly |
|   |  |
|  Per page migration:  | $5.25 |
|  Per 5 - 15 pages migration: Per page | $3.75 |
|  Per 16 - 40 pages migration: Per page | $3.5 |
|   |  |
### Other Services / Plugins
|  **Item / Description** | **Software Seni USD ** |
|  ------ | :------ |
|  Additional 3 plugins (from list)  | Hourly |
|   |  |
|  WordPress Multi-site  | Hourly |
|  Property Search Plugin | Hourly |
|  Real Estate listing management CRM  (Lava real estate manager plugin) | Hourly |
|  Email subscription plugin | Hourly |
|  Blog  | Hourly |
|  Calendar Plugin  | Hourly |
|  Job Board Site  | Hourly |
|   |  |
|  Any paid version of Woo Themes/ Woo Commerce plugin | Provided by Spotzer |
|  Production Resources /Hour | $17 |
|  SSL (and includes dedicated IP) per year | $98 |
|  Pretty permalinks with product names |  |
|   |  |
|  Ongoing site maintenaince edits/updates |  |
|  By the hour:(minimum 1 hours work) | just hourly rate above |
|  4 hours per month:  | just hourly rate above |
|  8 hours per month:  | just hourly rate above |
|   |  |
### Hosting
|  **Item / Description** | **Software Seni USD ** |
|  ------ | :------ |
|  Hosting | 12 per month |

### Possible WP Add-ons
|  **SoftwareSeni - Spotzer EU** |  |
|  :------ | :------: |
|  **Extra Feature / Function** | **USD($) / h** |
|  Possible extra plug-in, customization or functionality that a client may request.  | 17 |
|   |  |
|  **-- Site Features --** |  |
|  Customized Slider | 119 |
|  Custom Fields | 102 |
|  Lightbox |  |
|  Related Posts | 68 |
|   |  |
|   |  |
|  **-- Admin Panel Customization --** |  |
|  Custom Admin panel Branding | 42.5 |
|  Admin Panel functional set up  | hourly |
|  Add a Custom Dashboard Widget |  |
|   |  |
|  **-- Forms / Chat --** |  |
|  Customizable Forms | 34 |
|  Modal Contact Form(pop up window) | 68 |
|  On-Page Chat Functionality | 68 |
|   |  |
|  **-- Quote Generator --** | **hourly** |
|  **Quote List e-mail** |  |
|  Quote questionaire |  |
|  Quote ticket |  |
|  **-- Email Newsletter --** |  |
|  E-mail Newsletter package | 195 |
|  E-mail Newsletter(native) | 85 |
|  E-mail Newsletter(3rd Party) | 85 |
|  Newsletter Layout | 110 |
|  **-- Real Estate management --** |  |
|  Real estate management plugin | 275 |
|  Property search plugin | 510 |
|   |  |
|  **-- Dependant on clients shipping system --** |  |
|  Appointment Request/Booking | 204 |
|  Calendar Sync | 136 |
|  Events Calendar and booking | 204 |
|   |  |
|  **-- Customer Accounts / Social / Forums / Affiliates --** |  |
|  Customer Log-In Basic | 272 |
|  Customer Log-In Advanced |  |
|  Business Parter/ Vendor Log-in Extra privileges | 272 |
|  Social network Set up | 51 |
|  Forum Set up | 408 |
|  Affiliate Set-up | 170 |
|   |  |
|  **-- eLearning --** |  |
|  eLearning Platform | hourly |
|  Testing  |  |
|   |  |
|   |  |
|   |  |
|  **-- APIs --** |  |
|  API integration | hourly |
|   |  |
|  **-- SEO/SEM --** |  |
|  Yoast SEO plugin |  |
|   |  |
|  **-- MultiLingual --** |  |
|  Multilingual site set up | hourly |
|  4 Layout 20 page site in 2 languages |  |
|  4 Layout 20 page site in 3 languages |  |
|  5 Layout 50 page site in 2 languages |  |
|  5 Layout 50 page site in 3 languages |  |
|  Cost per each additional language |  |
|   |  |
|  **-- Redirects --** |  |
|  301 redirects | 68 |
|   |  |
|  **-- eCommerce Integration --** |  |
|  Magento Integration | 136 |
|   |  |
|  **-- Woocommerce --** |  |
|  Woo Commerce extension with 10 products configured | 488 |
|  Extra tabs on Product Page | 68 |
|  Currency Conversion | 170 |
|  Quoting Option | hourly |
|  Custom Product Search | hourly |
|  Per additionnal Custom Product Filters |  |
|  Basic Product search Config | 68 |
|   |  |
|   |  |
|  **-- Invoice / Email --** |  |
|  woocommerce invoice customization | hourly |
|  woocommerce email customization | hourly |
|   |  |
|  **-- Shipping --** |  |
|  Shipping table Rates | 34 |
|  Shipping Gateway | hourly |
|  UPS Shipping Gateway |  |
|  Fedex Shipping Gateway |  |
|  DHL Shipping Gateway |  |
|  Order Delivery Date Selector | 34 |
|   |  |
|  **-- Payment / Checkout / Tax --** |  |
|  Payment Gateway | hourly |
|  Added product fee at checkout |  |
|  Product option during Checkout | hourly |
|  Cart Customization | hourly |
|  Checkout Customization | hourly |
|  VAT tax compliant site | 68 |
|  Paypal integration | Included |
|  Worldpay integration | 102 |
|  Sage integration | 275 |
|   |  |
|  **-- Integration --** |  |
|  Enterprise Resource Planning Software Integration | hourly |
|  Customer Relationship manager software integration | hourly |
|  Amazon Integration | 952 |
|  eBay Integration | 952 |
|  Google shopping | 952 |
|   |  |
|  inSIte / Targeting | hourly |
|   |  |
|  **-- Multisite --** |  |
|  Mulitisite functionality |  |
|   |  |
|  **-- Server / Hosting / Security --** |  |
|  Hosting | TBC |
|  Server | TBC |
|  SSL (and includes dedicated IP) per year | 98 |
 
