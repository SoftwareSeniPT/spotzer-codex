...
var app = {
  ...
  detectHashChange: function() {
    ...
    function handleHashChange() {
      ...
      if (jQuery("[data-id=" + hash + "]").length) {
          ...
          jQuery("[data-id=" + hash + "]").parents(".main-category").addClass("opened").find('> ul').slideDown();;
      }
    }
    ...
    $(document).on('click', '[data-item-select]', function(event) {
      var id = $(this).attr('data-id');
      $(document).trigger('changeTopic', [id]);
    });
  ...
  changeTopic: function() {
    ...
    // Change topic by click on the menu
    jQuery(document).on('changeTopic', function(event, data) {
